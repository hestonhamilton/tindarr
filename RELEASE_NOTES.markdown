...
This release marks a significant milestone with a complete rebranding of the project from "MovieMatch" to "Tindarr". This change is reflected across the entire codebase, documentation, and deployment configurations.

### Key Changes:

- **Project Rebranding**: All instances of "MovieMatch" have been updated to "Tindarr" in code, configuration files, and user-facing documentation. This includes:
    - `package.json` and `package-lock.json`
    - Documentation files (`docs/configuration.md`, `docs/docker-compose.md`, `docs/reverse-proxy.md`)
    - Client-side UI elements and e2e tests (`packages/client/e2e/login.spec.ts`, `packages/client/index.html`, `packages/client/src/pages/AuthCallback.tsx`, `packages/client/src/pages/Login.tsx`)
    - Server-side messages (`packages/server/src/index.ts`)
    - Plex client identifiers (`X-Plex-Product`, `X-Plex-Client-Identifier`) in `packages/server/src/plex.test.ts` and `packages/server/src/plex.ts`.

- **GitHub Actions Workflow Enhancements**:
    - The `release.yaml` workflow has been substantially refactored for improved release management.
    - Tags now use a more generic `v*` pattern.
    - Node.js version updated to `22`.
    - Docker image name standardized to `hestonhamilton/tindarr`.
    - Updated to use `softprops/action-gh-release@v2` for GitHub releases and `docker/build-push-action@v5` for Docker image building and pushing.
    - Version determination and artifact handling have been streamlined.

- **Security Configuration**:
    - `.gitleaks.toml` title updated to "tindarr secret scan".

- **Development & Documentation**:
    - `TODO.md` updated to reflect the completion of the rebranding effort.
    - Issue templates (`.github/ISSUE_TEMPLATE/bug-report.md`) updated to reference "Tindarr" and new discussion links.
