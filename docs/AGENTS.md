...
This document outlines the best practices, workflows, and hard rules for AI Agents operating within this project. Adhering to these guidelines ensures efficient, safe, and consistent collaboration.
...
-   **Purpose:** The Context7 MCP server provides up-to-date documentation for various libraries and frameworks. It is crucial for understanding existing project dependencies and for researching new ones without upgrading versions unnecessarily.
-   **Workflow:**
    1.  **Resolve Library ID:** Before fetching documentation, use the `resolve_library_id` tool with the library's name (e.g., `resolve_library_id(libraryName='react')`). This will return a Context7-compatible library ID (e.g., `/facebook/react`).
    2.  **Fetch Documentation:** Once you have the library ID, use the `get_library_docs` tool with the obtained ID and, if applicable, a specific version or topic (e.g., `get_library_docs(context7CompatibleLibraryID='/facebook/react/v18.2.0', topic='hooks')`).
    3.  **Version Consistency:** Always prioritize using documentation for the versions currently used in the project. If a specific version is not available via `get_library_docs`, try to infer compatibility from the available versions or consult other reliable sources if explicitly permitted by the user.
    4.  **Avoid Unnecessary Upgrades:** Do not suggest or perform library upgrades unless explicitly instructed by the user or if it's a critical security fix and confirmed with the user.
